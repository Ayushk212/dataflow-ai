...
function BentoCard({ feature, isActive, onActivate }) {
  ...
  return (
    ...
    >
      ...
      {/* Title */}
      <h3 className="font-mono font-600 text-lg text-arctic leading-snug">
        {feature.title}
      </h3>

      {/* Description — visible on active or large */}
      <p
        className={`font-sans text-sm text-mystic leading-relaxed transition-all duration-300 ease-in-out
      ...
      <div className="mt-auto pt-4 border-t border-white/6 flex items-center justify-between">
        <div>
          <div className="font-mono font-700 text-xl" style={{ color: feature.accent }}>
            {feature.stat}
          </div>
    ...
    <div className="border border-white/8 rounded-xl overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="font-mono text-xs mb-0.5" style={{ color: feature.accent }}>
            {feature.eyebrow}
          </div>
          <div className="font-mono font-600 text-sm text-arctic truncate">
            {feature.title}
          </div>
        </div>
        ...
        {/* +/× icon */}
        <div
          className={`accordion-icon flex-shrink-0 w-7 h-7 rounded-full border flex items-center justify-center transition-colors duration-180
            ${isOpen ? 'border-forsythia bg-forsythia/10' : 'border-white/20'}`}
          aria-hidden="true"
        >
          <svg
            width="12"
      ...
      >
        <div className="px-5 pb-5 pt-1">
          <p className="font-sans text-sm text-mystic leading-relaxed mb-4">
            {feature.description}
          </p>
          <div className="flex items-center gap-3 pt-3 border-t border-white/6">
            <span className="font-mono font-700 text-lg" style={{ color: feature.accent }}>
              {feature.stat}
            </span>
...
export default function Features() {
  ...
  // Shared active index — a ref so bento hover doesn't cause layout re-render
  const activeIndexRef = useRef(0)

  // Bento desktop active state (causes only bento re-render, not parent)
  const [bentoActive, setBentoActive] = useState(0)

  // Accordion mobile open state
  ...
  // ResizeObserver — transfers context on breakpoint crossing
  useEffect(() => {
    const ro = new ResizeObserver(([entry]) => {
      ...
      if (nowMobile !== wasMobile) {
        if (nowMobile && !wasMobile) {
          ...
          // Mobile → Desktop: sync accordion open back to bento highlight
    ...
    >
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div ref={sectionRef} className="reveal mb-16 max-w-2xl">
          ...
          <h2
            id="features-heading"
            className="font-mono font-700 text-3xl md:text-4xl text-arctic leading-tight"
          >
